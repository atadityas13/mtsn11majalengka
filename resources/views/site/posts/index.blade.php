...
<div class="border-b border-kemenag/10 bg-kemenag-deep text-white">
    <div class="site-container py-12">
        ...
        @if ($activeYear && $activeMonth)
            @php
                $archiveLabel = \Carbon\Carbon::create((int) $activeYear, (int) $activeMonth, 1)
                    ->locale('id')
                    ->translatedFormat('F Y');
            @endphp
            <p class="mt-3 text-sm text-white/75">Arsip: {{ $archiveLabel }}</p>
        @endif
    </div>
</div>
...
<section class="site-container py-12 md:py-14" x-data="{ loading: false }" @submit="loading = true">
    <form method="get" action="{{ route('posts.index') }}" class="mb-8 flex flex-col gap-3 md:flex-row md:items-center">
        @if ($activeYear && $activeMonth)
            <input type="hidden" name="tahun" value="{{ $activeYear }}">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('posts.index', array_filter(['q' => $search ?: null, 'tahun' => $activeYear ?: null, 'bulan' => $activeMonth ?: null])) }}"
            ...
            @foreach ($categories as $category)
                <a href="{{ route('posts.index', array_filter(['q' => $search ?: null, 'kategori' => $category->slug, 'tahun' => $activeYear ?: null, 'bulan' => $activeMonth ?: null])) }}"
    ...
    @if ($activeYear && $activeMonth)
        <div class="mb-6">
            <a href="{{ route('posts.index', array_filter(['q' => $search ?: null, 'kategori' => $activeCategory ?: null])) }}" class="text-sm font-bold text-kemenag hover:underline">
                ← Semua berita
            </a>
        </div>
    @endif
    ...
    <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:gap-10">
        <div>
            <div x-show="loading" class="mb-6 grid gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
                @for ($i = 0; $i < 3; $i++)
            ...
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3" x-show="!loading">
        ...
        <aside class="article-sidebar space-y-5">
